  ...
  <section>
    <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="100">
      ...
      <FormItem label="文章关键字" prop="keyword">
        <Input v-model="formValidate.keyword" placeholder="关键字" />
      </FormItem>
      <FormItem label="文章分类" v-if="categoryList && categoryList.length > 0">
        <Select v-model="formValidate.category_id.name">
  ...
  methods: {
   ...
   async _updateArticle(){
    ...
    async _getArticleDetail(id) {
      let res = await this.getArticleDetail({id:id});
      //console.log(res);
      this.formValidate = res.data.data.article;
      this.category = this.formValidate.category_id.name;
    },
    // 上传图片成功
